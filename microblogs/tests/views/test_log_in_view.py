...
class LogInViewTestCase(TestCase,LogInTester):
    ...
    def test_get_log_in_with_redirect(self):
        destination = reverse('user_list')
        self.url = reverse_with_next('log_in',destination)
        response = self.client.get(self.url)
        self.assertEqual(response.status_code,200)
        self.assertTemplateUsed(response,'log_in.html')
        form = response.context['form']
        next = response.context['next']
        self.assertEqual(next,destination)
        self.assertTrue(isinstance(form,LoginForm))
        self.assertFalse(form.is_bound)
    ...
    def test_successful_log_in_with_redirectg(self):
        redirect_url = reverse('user_list')
        form_input = {'username':'Aniad123','password':'Password123','next':redirect_url}
        response = self.client.post(self.url,form_input,follow=True)
        self.assertRedirects(response, redirect_url, status_code=302,target_status_code=200)
        self.assertTemplateUsed(response,'user_list.html')
        self.assertTrue(self._is_logged_in())

    def test_post_log_in_when_logged_in(self):
        self.client.login(username='Aniad123',password='Password123')
        form_input = {'username':'Wrong123','password':'WrongPassword123'}
        response = self.client.post(self.url, form_input ,follow=True)
        self.assertRedirects(response,reverse('feed'),status_code=302,target_status_code=200)
        self.assertTemplateUsed(response,'feed.html')
